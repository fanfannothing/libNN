/*
 * NeuralNetworkGladys.hpp
 *
 *  Created on: Feb 19, 2012
 *      Author: wchan
 */

#ifndef NEURALNETWORKGLADYS_HPP_
#define NEURALNETWORKGLADYS_HPP_
...
class NeuralNetworkGladys : public NeuralNetwork {
public:
  NeuralNetworkGladys() {
    index = 0;
    ...
    m_network.reset(new NeuralNetworkMultilayerPerceptron( { DataSetMNIST::get_feature_size(), 128, DataSetMNIST::get_feature_size() }));
  ...
  void train_shallow() {
    ...
    for (int i = 0; i < 10; i++) {
      // train2.push_back(std::pair<boost::numeric::ublas::vector<double>, boost::numeric::ublas::vector<double> >(labels[i], labels[i]));
    }
  ...
  virtual boost::numeric::ublas::vector<double> f_shallow(boost::numeric::ublas::vector<double> x) {
    ...
    int wee = 0;
    png::image<png::rgb_pixel> image(28 * 100, 28);
...
#endif /* NEURALNETWORKGLADYS_HPP_ */
