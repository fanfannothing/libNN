...
RankSet DataSetStockFLRank::m_train;
RankSet DataSetStockFLRank::m_test;
std::vector<boost::numeric::ublas::vector<double> > DataSetStockFLRank::m_test_unsorted;

// TODO: right now it only uses fold1: this needs to be updated later...

void DataSetStockFLRank::load() {
  std::ifstream train("/home/wchan/research/flrank/stock-labels-train.csv", std::ifstream::in | std::ifstream::binary);
  std::ifstream test("/home/wchan/research/flrank/stock-labels-test.csv", std::ifstream::in | std::ifstream::binary);

  m_train.clear();
  m_test.clear();
  m_test_unsorted.clear();

  for (std::size_t i = 0; i < 100000 && train.good(); i++) {
    std::size_t query;
  ...
  for (std::size_t i = 0; i < 100000 && test.good(); i++) {
    ...
    m_test.push_back(query, features, label);
...
std::size_t DataSetStockFLRank::get_feature_size() {
  return 24;
}

std::size_t DataSetStockFLRank::get_label_size() {
  return 1;
}

RankSet DataSetStockFLRank::get_train() {
  if (m_train.size() == 0) {
    load();
  }
...
RankSet DataSetStockFLRank::get_test() {
  if (m_test.size() == 0) {
    load();
  }
  return m_test;
