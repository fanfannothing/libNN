...
#ifndef NEURALNETWORKMULTILAYERCU_HPP_
#define NEURALNETWORKMULTILAYERCU_HPP_
...
class NeuralNetworkMultiLayerCU : public NeuralNetworkCU {
public:
  /* the first entry is for the input layer... therefore a "two-layer" network actually has 3 entries */
  NeuralNetworkMultiLayerCU(std::vector<size_t> size) {
    assert(size.size() >= 2);
  ...
  NeuralNetworkMultiLayerCU(std::shared_ptr<NeuralNetworkMultiLayer<ActivationFunction> > network) {
  ...
  virtual ~NeuralNetworkMultiLayerCU() {
  }
